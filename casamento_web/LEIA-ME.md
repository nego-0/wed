...
| `assets/bar-montagem.js`, `assets/bar-copa.js`, `assets/bar-entrega.js`, `assets/bar-convidado.js` | Um por ecrã. A montagem tem cinco separadores: o menu, as gavetas, as folhas de QR, **as Regras do Bar** e **a equipa** — as contas de copeiro e de garçom criam-se ali, ao lado do ecrã onde vão trabalhar. |
| `assets/bar-regras.js` | **As Regras do Bar**, num painel só: como a casa se porta, os limites e intervalos (agrupados por alcance — o caudal da copa, toda a gente, um convite, uma pessoa) e os motivos de recusa. O **mesmo** ficheiro é montado na montagem e na copa: duas cópias parecidas do mesmo ecrã foi como as regras começaram a divergir. |
| `assets/icones.js` | **Os desenhos da casa**, num sítio só: seis copos (taça, flute, caneca, copo baixo, copo alto, chávena) e ~30 sinais de interface, todos 24×24 a traço e em `currentColor` — atravessam os quatro temas sem uma regra a mais. `ICO.copo('Café', 'Sem álcool')` escolhe o copo pelo nome da bebida, e só depois pela gaveta. Zero emojis no módulo. |
