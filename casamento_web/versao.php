...
function correcoesEsperadas(): array {
    return [
        ...
        ['A galeria traz quatro conjuntos fictícios completos derivados da linguagem das fotografias de origem',
         'personalizacao.php', "'capa-exemplo.jpg'       => 'Casal de exemplo · capa'"],
        ['Com alternativas urbanas, botânicas e de deserto, cada uma com quatro secções',
         'personalizacao.php', "'capa-aurora-urbana.jpg'       => 'Aurora Urbana · capa'"],
        ['E são essas variações, não o casal real, que alimentam os dados de exemplo',
         'personalizacao.php', "'media.hero'       => 'assets/convite/galeria/capa-exemplo.jpg'"],
        ['Referências antigas às fotografias removidas passam para a variação da mesma secção',
         'personalizacao.php', 'function fotosGaleriaAtuais(array $defs): array {'],
        ['A proveniência e a função dos cinco conjuntos ficam documentadas',
         'assets/convite/galeria/CREDITOS.md', '## Fotografias geradas — Dunas de Cobre'],
        ...
        ['Bar: o menu monta-se na página dos noivos, com fotografia e stock',
         'bar.php', "barLinkDaFesta(\$conn)"],
        ['Bar: as regras do bar guardam-se fora do vocabulário do convite',
         'db.php', 'function barGuardarDefs('],
        ['Bar: o convidado entra pelo endereço da festa, sem sessão nem link no convite',
         'bebidas.php', 'barCasamentoDoToken($conn, $tokenCasa)'],
        ...
        ['A porta pública do bar aceita o código da festa ao lado do da mesa',
         'api.php', "\$casa = trim((string)(\$_GET['c'] ?? (corpo()['c'] ?? '')));"],
        ...
        ['Cada festa tem um endereço curto: o ano e as iniciais dos noivos',
         'db.php', 'function barSlugDe(string $data, string $noiva, string $noivo,'],
        ['E só cresce quando choca: entra o mês, o dia, a hora ou o local',
        ...
        // ---- «Copos por garrafa» não se aplica a quem só serve garrafas ----
        ['O servidor anula «copos por garrafa» na bebida que só sai à garrafa',
         'api.php', "\$dose = \$serv === 'garrafa' ? 1 : max(1, min(60, (int)(\$d['doses_garrafa'] ?? 6)));"],
        ['E também à leitura, para um registo antigo não voltar a descontar seis',
         'api.php', "\$x['doses_garrafa'] = \$x['servir'] === 'garrafa'"],
        ['As que já estavam gravadas passam a uma: uma garrafa custa uma garrafa',
         'db.php', "UPDATE {\$P}bar_itens SET doses_garrafa=1 WHERE servir='garrafa'"],
        ['O formulário deixa de perguntar o que não se aplica, e diz a unidade do stock',
         'assets/bar-montagem.js', 'function bebidaReactiva(f) {'],

        // ---- A regra do bar conta artigos, como o consumo que lhe serve de conta ----
        ['O tecto de uma regra é de ARTIGOS, e não de doses',
         'api.php', "\$out['max_itens'] = min(\$out['max_itens'], \$sobra);"],
        ['Sem isto, «no máximo 2 bebidas» apagava o «+» de qualquer garrafa',
         'api.php', '// A regra ficou deste lado porque é deste lado que ela já se media:'],
        ['E a copa faz as mesmas contas que o telemóvel fez antes do pedido entrar',
         'api.php', "if ((int)\$f['q'] > (int)\$v['max_itens']) {"],
        ['O que falta de cada limite mede-se onde o limite vive',
         'api.php', "\$cabe = min((int)\$i['max_itens'], (int)\$i['pode_pedir']);"],
        ['A fila por decidir volta a assinalar os pedidos que saíram das regras',
         'api.php', "\$quanto = \$li['quantidade'] > \$v['max_itens'] ? (int)\$li['quantidade']"],
        ['E a recusa promete o que a casa serve mesmo, e não o que há no armazém',
         'api.php', "\$aceites = max(0, min((int)\$v['pode'], (int)\$v['max_itens']));"],
        ['A copa vê o tecto de cada linha em artigos, que é como ela decide',
         'api.php', "\$pode = max(0, min((int)\$v['max_itens'], intdiv((int)\$v['pode'], \$dosesLi)));"],

        // ---- O que se pinta no escuro tem de virar com ele ----
        ['O verde de «está tudo sentado» é um token, e não um verde cravado',
