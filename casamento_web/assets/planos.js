...
(function (global) {
  ...
  var sel = { pacote: 0, escaloes: {}, prazo: 0, aMedida: false };
    ...
    escolha: function () {
      ...
      if (sel.pacote) {
        ...
        if (p) return { pacote: p.id, escaloes: p.itens.slice(), meses: meses,
                        total: comPrazo(p.preco), vazio: false };
      }
      var ids = [], total = 0;
      ...
      return { pacote: 0, escaloes: ids, meses: meses, total: total,
               vazio: ids.length === 0 };
    },

    /** Marca uma escolha vinda de fora (um pedido já submetido, a reabrir). */
    repor: function (pacoteId, escalaoIds, meses) {
      sel = { pacote: 0, escaloes: {}, prazo: meses || 0,
