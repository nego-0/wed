...
function licSelLigarUm(cx){
    ...
    /**
     * Pôr a lista onde ela caiba — SEM mexer na janela.
     *
     ...
     *
     * Houve aqui um terceiro gesto, e foi tirado: quando não havia espaço em
     * baixo, isto ROLAVA o corpo da janela para o ganhar. Resolvia o corte, e
     * criava coisa pior — a janela mexia-se debaixo dos olhos de quem tinha
     * acabado de carregar no campo. Tudo o que estava à volta (o rótulo, os
     * outros campos, o botão de guardar) saltava de sítio por se ter aberto uma
     * lista, e o que a pessoa estava a ler fugia-lhe. Um menu que se abre não
     * pode arrastar a página consigo.
     *
     * Ficam dois gestos, e nenhum toca no que está por baixo: escolher o LADO
     * onde há mais espaço, e apertar a altura ao que esse lado tem. Prefere-se
     * abrir para baixo enquanto der, porque é onde a pessoa espera a lista
    ...
    const assentar = () => {
      ...
      const abaixo = j.bottom - b.bottom - RESPIRO;
      const acima  = b.top - j.top - RESPIRO;

      // O que a lista quer, e o que a rodeia dentro da caixa (a procura, e o
      // «nada com esse nome»). Aperta-se a LISTA, nunca o conjunto: a caixa de
      // procura tem de ficar à vista mesmo quando só cabem duas linhas.
      const fora = pop.offsetHeight - lista.offsetHeight;
      const quer = lista.scrollHeight + fora;
      ...
      const forro = (parseFloat(getComputedStyle(lista).paddingTop) || 0) * 2;
      const inteiras = (h) => (linha > 0
        ? Math.max(linha + forro, Math.floor((h - forro) / linha) * linha + forro)
        : Math.max(80, h));

      // Para cima só quando vale a pena: em baixo não chega, e em cima chega
      // MESMO mais — uma linha inteira mais, pelo menos. Sem essa margem, um
      // campo a meio da janela trocava de lado a cada dois pixels de rolagem,
      ...
      /* E na horizontal.
         ...
         o campo (ver .lic-sel-pop, em janela.css) — o que num campo encostado à
